...
| 3. Supabase (staging) | Projeto novo, **schema correto**, e-mail confirmado | `alembic current` (com a `DATABASE_URL` de staging) batendo com o head do repositório na hora da checagem — `b77a9b5656c2` quando o banco fechou em 30/08/2026; a Seção 4 levou o head do repositório para `9b0c34de353b` e staging recebeu esse deploy em 06/09/2026, e a **Seção 7 levou para `170b12223b9b`** (a migração das tabelas de telemetria), com **staging recebendo em 11/09/2026** — verificado de fora desta vez, não só inferido: `POST /api/telemetry/events` sem token responde `422` e a rota aparece no `openapi.json`, o que prova que o contêiner serve o código da seção; pela ADR 0007, contêiner no ar implica receita aplicada. Produção continua atrás até `main` receber o merge. São **três** números diferentes nesta linha, de três momentos diferentes — não repita nenhum deles sem rodar `alembic heads` (repositório) ou `alembic current` com a `DATABASE_URL` do ambiente (implantado); em *Authentication → Providers → Email* do painel, "Confirm email" ligada |
