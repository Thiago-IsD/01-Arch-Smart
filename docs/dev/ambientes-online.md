...
| 3. Supabase (staging) | Projeto novo, **schema correto**, e-mail confirmado | `alembic current` (com a `DATABASE_URL` de staging) batendo com o head do repositório na hora da checagem — `b77a9b5656c2` quando o banco fechou em 30/08/2026; a Seção 4 acrescentou migrações e levou o head do repositório para `9b0c34de353b`, e **staging recebeu esse deploy em 06/09/2026** (contêiner no ar servindo o código novo, o que pela ADR 0007 implica receita aplicada). Produção continua atrás até `main` receber o merge. Não repita nenhum dos dois números sem rodar `alembic heads` (repositório) ou `alembic current` com a `DATABASE_URL` do ambiente (implantado); em *Authentication → Providers → Email* do painel, "Confirm email" ligada |
