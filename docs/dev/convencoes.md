...
| Teste que substitui a sessão de banco por `MagicMock`/mock equivalente para validar comportamento que depende de dado real (isolamento entre contas, filtro por `account_id`, contagem de linhas) | 1 | A suíte antiga (`app/tests/`, 63 testes) usa exatamente esse padrão e não detectou nenhum dos 14 vazamentos entre contas encontrados na auditoria de 23/08/2026 — porque um mock não tem conceito de "linha de outra conta" para vazar. Está fora do `testpaths` do `pytest.ini`, será apagada na Seção 4, e nada novo deve seguir esse padrão. Teste que prova isolamento roda contra Postgres real (`tests/isolation/`, 29 testes hoje). |
