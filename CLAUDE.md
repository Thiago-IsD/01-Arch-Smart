...
15. **Não existe `not-found.tsx` na árvore da aplicação — achado na revisão
    final da branch, fora do escopo desta migração.** Uma rota que não casa
    com nenhum segmento cai no 404 padrão do Next: página em inglês, fora do
    shell do produto — diferente do `notFound()` deliberado do item 3 acima,
    que roda dentro do `<Suspense>` do detalhe e responde `200` com a UI de
    "não encontrado" do próprio produto. Medido: `find ArchSmart-web/src
    -iname "not-found*"` → nenhum arquivo (16/09/2026). A verificação humana
    da Tarefa 12 não olhou esta página — não estava na lista de 8 perguntas
    entregue. Não corrigido, de propósito: criar o arquivo é decisão de
    design (o que mostrar, em que idioma, com ou sem shell), fora do escopo
    de arquivos desta migração.
