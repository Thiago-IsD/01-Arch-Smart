...
- ~~**O badge do inbox (`useInboxCount()`) nunca é prefetchado**~~ — pendência
  ...
  custo puro). ✅ **Confirmado ao vivo em 12/09/2026: o `state=CAPTURED`
  parou de sair do navegador.** Três execuções consecutivas de
  `hidratacao-biblioteca.spec.ts`, instrumentado para listar **todos** os
  pedidos, deram `PEDIDOS_TOTAL=0` / `PEDIDOS_NORMALIZED=0` /
  `PEDIDOS_CAPTURED=0` — zero pedido a `/api/products` de qualquer tipo no
  primeiro carregamento. O prefetch do badge **está sendo aproveitado**, e a
  asserção foi apertada no mesmo commit, como esta nota mandava: o spec deixou
  de filtrar por `state=NORMALIZED` e exige zero pedido de qualquer tipo, com a
  distinção entre lista e badge movida para a mensagem de falha.
...
2. ~~**A prova viva do `screen_viewed` continua sem rodar, pela mesma parede.**~~
   ✅ **FECHADA em 12/09/2026.** `telemetria-biblioteca.spec.ts` rodou pela
   primeira vez e **passou**, em três execuções consecutivas, sem que nenhuma
   asserção fosse afrouxada — o spec estava correto como escrito. E a linha
   chegou ao banco: `product_events` estava com **0 linhas** (medido, não
   suposto) e passou a ter linhas de `/library` com `medido_ate=dados`,
   `medido_de=clique`, `principal_declarada=true`, `is_empty=false` e `load_ms`
   de mediana **1068 ms** (n=21) — mesma ordem de grandeza da mediana de
   **1415 ms** do E2E, e longe do `load_ms: 28` da Seção 7. **O `load_ms` é
   dado utilizável**, com uma ressalva que a própria medição tornou concreta:
   `/dashboard`, que a Seção 8 não migrou, grava mediana de **18 ms** com
   `medido_ate=pintura` — **agregue sempre filtrando `medido_ate`**. Números e
   comandos em
   [`docs/dev/medicoes/2026-09-06-biblioteca-depois.md`](docs/dev/medicoes/2026-09-06-biblioteca-depois.md).
   O texto original fica abaixo porque a **forma** dele continua valendo — o
   defeito que ela ia expor foi consertado, e o spec que a fecha já existia:
   ...
   **Esse spec nunca foi executado**, e está no quarto job do CI — que hoje roda sob demanda (`gh workflow run e2e.yml`): é lá, ou à mão
   ...
   >
   > **Uma ressalva, e ela mudou de forma em 12/09/2026.** O corte entre o
   > gatilho antigo e este é um **evento, não uma data — é o deploy**; isso
   > continua valendo. O que mudou: `product_events` em staging **tem** linhas do
   > protocolo novo, e elas **não vieram do deployment de staging** — vieram de
   > front e API rodando na máquina de desenvolvimento, com o código da Seção 8,
   > apontados para o **banco** de staging. Ou seja: a tabela tem dado do
   > protocolo novo enquanto o ambiente ainda serve o antigo. Quem cruzar "linha
   > recente" com "deploy de staging" erra nas duas direções — confira **de onde
   > a linha veio**, não a data dela. A grandeza do número, que era a parte
   > aberta, **foi medida** (mediana de 1068 ms; ver o ✅ no item 2 acima).
...
1. ~~⚠️ **A credencial do usuário de teste E2E está sendo rejeitada pelo
   Supabase de staging.**~~ ✅ **A PAREDE CAIU em 12/09/2026.** Thiago corrigiu
   a credencial — tinha um caractere sobrando — e ela foi verificada direto no
   endpoint de auth antes de qualquer medição: **`HTTP 200`**, token recebido,
   `email_confirmed_at: 2026-09-10T11:36:43Z`. A senha vive em
   `ArchSmart-web/.env.e2e.local` (não versionada) e é carregada com
   `set -a; . ./.env.e2e.local; set +a`. **O que a queda da parede fechou:** o
   orçamento de performance (1415 ms, contra 1454 ms de antes), a prova viva do
   `screen_viewed` (3/3), a confirmação de que o badge do inbox saiu do
   navegador, e o P95 de `/api/products` (634 ms, estourando o orçamento por
   causa do JWT e não da query). **O que a queda da parede NÃO fechou:** os três
   itens que dependem de olho humano (axe em navegador, teclado, 390px/1440px) e
   a verificação visual da Seção 6 — agora **executáveis**, já que
   `e2e/captura-visual-secao-6.spec.ts` existe e a credencial funciona, mas
   ninguém olhou captura nenhuma ainda. Os Secrets do repositório, que o job
   `e2e` do CI espera, **continuam sem existir** — o job segue sob demanda. O
   texto original fica abaixo como histórico do que a parede custou. Enquanto
   ela durou: nenhuma tela consegue fechar os três itens da
...
11. **`QueryBoundary` com query desabilitada: hoje há aviso, e falta desenho.**
    No react-query v5 uma query com `enabled: false` fica `status: "pending"` /
    `fetchStatus: "idle"` para sempre, então o boundary renderiza **skeleton
    eterno**. A Biblioteca escapa por construção — ela não renderiza o boundary
    quando a aba não precisa da lista —, e a Seção 8 fez o mínimo: o boundary
    **não anuncia** nesse estado (para a telemetria parar de gravar `abandonado`
    falso), com a regra escrita na docstring e teste dos dois lados. O que **não**
    foi feito, de propósito, é criar API para o caso: um prop explícito
    (`inativo`, ou equivalente) que diga o que renderizar quando a query está
    desligada. Isso é desenho, e `features/library/hooks.ts` já tem **quatro**
    queries gateadas por `enabled` — query dependente é o caso comum das oito
    telas seguintes, e a segunda cópia é onde o esquecimento aparece. Medir:
    `grep -n "enabled" ArchSmart-web/src/features/*/hooks.ts`.

## O que o Dashboard (Seção 8) deixou em aberto — **planejar antes da próxima tela**
...
1. **A distância entre a API e o banco continua sem decisão.** O Dashboard
   fechou com P95 de **1915 ms** contra 400 ms, e toda tela seguinte vai
   registrar o mesmo "estoura por distância, não pela tela" enquanto a topologia
   não mudar. As três saídas e o custo de cada uma estão na tabela "As saídas"
   de
...
5. **A Biblioteca estoura 42px em 390px**, nos dois temas. A causa foi isolada
   ...
   linha do título — **não** a toolbar. Não foi consertado: é layout de tela já
   migrada, e só entra com ok de Thiago.

6. **"Plano Solo" é texto fixo** em `ProjectsLimitCard.tsx:31` — nome de plano
   decidido no front, possível violação do Art. 3. O nome real vem de
...
4. ~~**`GET /api/projects/{id}` calcula `custom_installments` e o Pydantic
   ...
   recebimento Personalizado, não é só limpeza de backend.**~~ **Fechado em
   16/09/2026, na branch de limpeza `secao-8-limpeza-projetos`, em dois
   commits.** `c289bd1` fez `ProjectResponse` declarar
   `custom_installments: Optional[List[CustomInstallment]] = None`, então o
   campo para de ser descartado na serialização. `a2197d0` foi além do que
   este item pedia: a consulta do detalhe passou a trazer **todas** as
   parcelas (previstas e recebidas, não só `PREDICTED`), e
   `sync_project_financials` (ramo CUSTOM) passou a **preservar** as
   `REALIZED` intocadas e re-sincronizar só as previstas, casando o array de
   entrada contra as recebidas por `(amount, due_date)` — sem isso, reenviar
   o mesmo cronograma no PUT duplicava a parcela já recebida a cada salvamento.
   **O que ficou em aberto:** o casamento por `(amount, due_date)` é
   heurístico — se o usuário editar deliberadamente o valor ou a data da
   linha que representa a parcela **já recebida** no wizard, ela deixa de
   casar com a `REALIZED` existente e uma `PREDICTED` nova é criada a mais
   (não duplica dinheiro recebido, mas produz uma parcela extra não
   intencional). A saída seria travar essa linha na interface — **decisão de
   produto, de Thiago**, não tomada aqui. Ver
...
8. ~~⚠️ **Achado novo: `/projects/{id}` estoura 203px em 390px, nos dois
   ...
   593px), não `ProjectHeader.tsx` (a fileira de abas já tem `flex-wrap`)....~~
   **Fechado em 16–17/09/2026, na branch de limpeza `secao-8-limpeza-projetos`
   — e a atribuição acima ("o culpado é o shell") estava só parcialmente
   certa.** Medido: **151 dos 203px** eram a própria barra de **ações** do
   ...
   `564af1b` (`flex-wrap gap-2`), que baixou o estouro para 52px. Os 52px
   restantes **não** fecharam com `min-w-0` isolado em `Header.tsx` (medido:
   zero efeito, 52px antes e depois de aplicar só essa mudança) — a causa
   real era `AppShell.tsx:89` (`flex-1 flex flex-col`, sem `min-w-0`), que
   impedia a coluna inteira do shell de encolher abaixo do seu conteúdo.
   Corrigida no commit `413c8f8` (`min-w-0` em `AppShell.tsx` e no grupo
   esquerdo de `Header.tsx`, `shrink-0` no botão de menu), o estouro caiu a
   **zero** em `/projects`, `/projects/<id>`, `/dashboard` e `/library`, nas
   duas larguras e nos dois temas — a mesma correção fechou de brinde o
   estouro de 42px da Biblioteca (mesma causa raiz, não o `p-8` de
   `library/page.tsx`, que continua lá como redundância inofensiva). O
   instrumento versionado
   [`docs/dev/medicoes/2026-09-17-largura-do-shell.md`](docs/dev/medicoes/2026-09-17-largura-do-shell.md)
   confirmou, numa rodada seguinte, que a correção não empurrou o excesso
   para rolagem **dentro** de `main` (invisível na métrica do documento): 8
   rotas, 2 larguras, `MAIN_DIFF=0` nas 16 combinações. Números em
...
14. ~~⚠️ **Cada defeito que a verificação humana da Tarefa 12 apontar, e que
    não for consertado no mesmo ciclo, entra aqui.**~~ **Fechada em
    16/09/2026: a resposta de Thiago à lista de verificação
    (`.superpowers/sdd/2026-09-15-secao-8-projetos/task-12-report.md`) foi
    "verificado" — uma aprovação global da branch inteira, sem apontar
    nenhum defeito e sem comentário item a item dos 16 pontos entregues (4
    rotas — `/projects`, `/projects/<id>`, `/dashboard`, `/library` — × 2
    larguras × 2 temas, mais 8 perguntas dirigidas).** Isso é diferente de
    "cada item foi revisto e aprovado individualmente", e a diferença é
    registrada de propósito: **nenhum item novo entra nesta lista por causa
    da verificação humana**, e os itens já conhecidos — 3 (`notFound` 200),
    4 (`custom_installments`), 6 (contraste), 7 (axe do detalhe), 8 (estouro
    de 203px), 9, 10, 12, 13, mais os itens 15 e 16 abaixo — seguem
    registrados como **conhecidos e aceitos**, não confirmados um a um.

15. **Existe `not-found.tsx` em `projects/[id]` desde 16/09/2026 (branch de
    limpeza `secao-8-limpeza-projetos`, commits `2011353` + `6ea0a36`) — mas
    continua faltando um **de raiz**, e essa parte do achado original segue
    aberta.** O arquivo em `projects/[id]/not-found.tsx` é o boundary de
    `notFound()` para o segmento inteiro e todo descendente sem `not-found.tsx`
    próprio — **6 chamadas em 5 arquivos**, não só a do detalhe (`grep -rn
    "notFound()" "ArchSmart-web/src/app/(dashboard)/projects/[id]"
    --include=*.tsx`, comando no próprio docstring do arquivo): detalhe,
    orçamento (quando o orçamento ainda não existe), apresentação, impressão
    e o construtor de apresentação (que chama duas vezes). A copy é
    deliberadamente genérica ("Isto pode não existir, ainda não ter sido
    criado, ou não pertencer a esta conta.") para não mentir em nenhum dos
    cinco casos, e o shell continua renderizado (diferente do 404 embutido do
    Next). **O que continua faltando**: uma rota que não casa com **nenhum**
    segmento da aplicação (`/projetos-typo`, por exemplo) ainda cai no 404
    padrão do Next — página em inglês, fora do shell do produto — porque não
    existe `not-found.tsx` na **raiz** da árvore (`src/app/not-found.tsx`).
    Medido: `find ArchSmart-web/src -iname "not-found*"` → só o arquivo de
    `projects/[id]` (17/09/2026). Não corrigido, de propósito: criar o de
    raiz é decisão de design (o que mostrar, em que idioma, com ou sem
    shell), fora do escopo desta limpeza.

16. **A copy dos erros de mutação (toasts de falha das sete mutações de
    Projetos) nunca foi revisada por olho humano.** Achado da revisão final
    da branch: a mensagem vem direto de `ApiError`/de uma mensagem genérica,
    sem passar por revisão de tom ou clareza. A verificação humana da Tarefa
    12 não cobriu esta pergunta — não estava na lista de 8 entregue. Não
    corrigido.
...
> `medicao-largura` — acrescentado em 17/09/2026, na limpeza de Projetos, mede
> estouro do documento **e** dentro do primeiro `<main>`, parametrizado por
> `ROTA`/`LARGURA`/`TEMA` —, `captura-visual-secao-6`), que produzem número e imagem,
...
**Desde 12/09/2026 a subida aceita também é gravada dentro do arquivo**, em `_pioras_aceitas` (chave de documentação, prefixo `_`, invisível para `medir()`/`comparar()`), pela própria ferramenta: `--atualizar --aceitar-piora` escreve `de`, `ate` e o commit corrente, e deixa `motivo` em branco dizendo na saída que ele precisa ser preenchido. Ninguém edita esse JSON à mão — esse é justamente o hábito que a guarda existe para impedir. **A comparação com a branch base só aceita a subida que um registro cobrir**, por **quatro** condições, todas fail-closed: as três de `registro_cobre_piora` em `tools/catraca.py` — existe registro para aquela chave, `de` é igual ao valor da branch base, e o valor desta branch é `<= ate` — mais a quarta, em `_auditar_baseline`: **o baseline desta branch tem que ser igual à medição de hoje**. Sem registro, com `de` de outra transição, acima do teto, ou com o baseline divergindo da medição, reprova. O campo é `ate` (teto) e não `para` (valor exato) porque o caminho normal depois de uma subida aceita é o número voltar a descer sem ainda alcançar o da base.

> **Por que a quarta condição existe, e por que é o registro que a carrega.** Com só as três primeiras, o teto deixava um buraco do tamanho da diferença entre ele e a medição: com `cores_literais` aceito `ate: 588` e medindo 583, dava para editar o baseline à mão para 585 e **os dois portões passavam** — a auditoria porque `585 <= 588`, e o portão principal porque a invariante dele é `medição <= baseline` e ele lia isso como "baixou de 585 para 583", saída 0. Cinco cores literais novas caberiam escondidas ali. Teto estreita, não fecha; quem fecha é a conferência contra a medição, e ela pesa sobre a medida **com** registro porque é o registro que abre a porta. Fora daí o modo continua não medindo nada — comparar baseline com baseline é o certo para o resto, e é o que o mantém barato. Se a medida com registro **não puder** ser medida naquela execução (`eslint_erros` sem `--eslint-json`, por exemplo), o comando reprova dizendo isso, em vez de aceitar por omissão.
...
Sai limpo: `Test Files 32 passed (32)`, `Tests 247 passed (247)` (medido em 15/09/2026, no resultado do merge do Dashboard em `develop`; eram 29/236 depois da Biblioteca, em 12/09/2026, e 20/162 depois da Seção 7. O número sobe quando uma seção acrescenta testes, então **meça, não copie daqui**). Um `failed` em qualquer das duas linhas é um teste quebrado de verdade. O backend, no mesmo dia, sai `360 passed, 1 skipped` — e **só com o Postgres de teste de pé**: com o Docker Desktop parado a suíte sai com 164 `errors` de `Connection refused` na porta 55432, que não é defeito de código (medido em 15/09/2026).
