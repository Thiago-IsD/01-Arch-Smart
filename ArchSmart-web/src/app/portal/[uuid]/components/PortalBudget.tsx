...
export function PortalBudget({ initialItems, environments, presentationId, status }: PortalBudgetProps) {
    ...
    // Justificativa inline ao recusar um item
    ...
    const handleRejectOption = async (itemId: string, optionId: string, reason: string) => {
        if (isLocked) return;
        ...
        setItems(prev => prev.map(item => {
            ...
            return {
                ...
                options: item.options.map(opt => {
                    ...
                    return { ...opt, approval_status: "REJECTED", rejection_reason: trimmedReason || null };
                })
            };
        }));

        // Fecha o formulário de justificativa
                                    ...
                                    return (
                                        <div key={item.id} className="p-4">
                                            ...
                                            <div className="flex items-center justify-between border-t border-slate-100 pt-3 mt-3">
                                                ...
                                                {!isLocked && rejectingItemId !== item.id && (
                                            ...
                                            {/* Justificativa salva (quando recusado) */}
                                            {activeOption?.approval_status === "REJECTED" && activeOption?.rejection_reason && rejectingItemId !== item.id && (
                                                <p className="text-xs text-rose-600 bg-rose-50 border border-rose-100 rounded-lg px-3 py-2 mt-2">
                                                    <span className="font-semibold">Motivo da recusa:</span> {activeOption.rejection_reason}
                                                </p>
                                            )}

                                            {/* Formulário inline de justificativa */}
                                            {!isLocked && rejectingItemId === item.id && activeOption && (
                                                <div className="mt-3 border-t border-slate-100 pt-3 space-y-2">
                                                    <label htmlFor={`reject-reason-${item.id}`} className="block text-xs font-semibold text-slate-600">
                                                        Por que está recusando este item? <span className="font-normal text-slate-400">(opcional)</span>
                                                    </label>
                                                    <textarea
                                                        id={`reject-reason-${item.id}`}
                                                        value={rejectReason}
                                                        onChange={(e) => setRejectReason(e.target.value)}
                                                        placeholder="Ex: prefiro outra cor, valor acima do esperado..."
                                                        className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2 text-sm min-h-[70px] outline-none focus:border-rose-400 focus:ring-1 focus:ring-rose-400 transition-all"
                                                    />
                                                    <div className="flex justify-end gap-2">
                                                        ...
                                                        <button
                                                            onClick={() => handleRejectOption(item.id, activeOption.id, rejectReason)}
